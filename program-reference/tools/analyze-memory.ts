...
function analyzeMemoryReport(filePath: string): void {
  console.log(`\n📊 Analyzing memory report: ${path.basename(filePath)}\n`);
  ...
  if (mainHeapMB > 100) {
    insights.push(`⚠️  Main process heap is ${mainHeapMB.toFixed(0)}MB - check for data accumulation`);
  }

  // Check number of renderers
  if (report.renderers.length > 5) {
    insights.push(`📌 ${report.renderers.length} renderer processes - each overlay window uses memory`);
  }
  ...
  for (const renderer of report.renderers) {
    ...
    if (memMB > 150) {
      insights.push(`⚠️  "${renderer.title}" using ${memMB.toFixed(0)}MB - may have DevTools open or memory leak`);
    }
  }
  ...
  if (actualTotal > expectedTotal * 1.5) {
    insights.push(`⚠️  Memory usage (${actualTotal.toFixed(0)}MB) is higher than expected (~${expectedTotal.toFixed(0)}MB)`);
  ...
  if (insights.length === 0) {
    console.log('✅ Memory usage looks normal for the number of windows');
...
function showHelp(): void {
  console.log(`
╔═══════════════════════════════════════════════════════════════╗
║            ELECTRON MEMORY PROFILING GUIDE                    ║
╚═══════════════════════════════════════════════════════════════╝
...
📋 QUICK REFERENCE
  ...
  ├── GPU Process:      50-150 MB
  ├── Each Renderer:    80-150 MB
  └── DevTools (open):  100-300 MB extra per window
...
2. TAKE HEAP SNAPSHOT (Main Process)
   - Tray menu → Memory → Take Heap Snapshot
...
═══════════════════════════════════════════════════════════════
📊 ANALYZE A MEMORY REPORT
═══════════════════════════════════════════════════════════════

Usage: node tools/analyze-memory.ts <memory-report.json>

Generate a report via: Tray → Memory → Save Memory Report
